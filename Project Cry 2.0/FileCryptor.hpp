...
private:
	std::unique_ptr<ProjectCry::ICryptor> crypting_algorithm;
...
public:
	...
	FileCryptor();
	FileCryptor(std::unique_ptr<ProjectCry::ICryptor>&& cryptor) noexcept;
	FileCryptor(FileCryptor&& fileCryptor) noexcept;
	~FileCryptor() = default;

	void set_crypting_algorithm(std::unique_ptr<ProjectCry::ICryptor>&& cryptor) noexcept;
