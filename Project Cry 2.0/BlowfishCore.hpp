...
private:
    uint32_t _p_block[18] = { 0x2, 0x4, 0x3, 0xF, 0x6, 0xA, 0x8, 0x8, 0x8, 0x5, 0xA, 0x3,  0x0, 0x8, 0xD, 0x3, 0x1, 0x3 };
    uint32_t _sbox[4][256] = {
    ...
    inline auto _split_block(const uint8_t* block, uint32_t& f, uint32_t& s)                    -> void;
    inline auto _xor_p_block_with_key(const char* key, const size_t key_size)                   -> void;
    inline auto _key_extansion(const char* key)                                                 -> void;
    inline auto _key_encryption()                                                               -> void;
    inline auto _sbox_encryption()                                                              -> void;
    inline auto _round(uint32_t& block1, uint32_t& block2, const uint32_t& r_key)               -> void;
