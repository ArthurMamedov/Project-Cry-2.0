...
auto GostCore::inv_cry_round(uint8_t* block) -> void {
	...
	for (uint8_t c = 0; c < GOST_BLOCK_LENGTH; c++) {
		_round(&N1, &N2, c);
	}

	for (uint8_t c = 31; c >= GOST_BLOCK_LENGTH; c--) {
		_round(&N1, &N2, c);
	}
...
inline auto GostCore::_round(uint32_t* block32b_1, uint32_t* block32b_2, uint8_t i) -> void {
	...
	rnd = (*block32b_1 + _round_keys[i % GOST_BLOCK_LENGTH]);

	rnd = _substitution_table(rnd, i % GOST_BLOCK_LENGTH);
...
inline auto GostCore::_split_256bits_to_32bits(const char* key256b) -> void {
	size_t inc = 0;
	for (uint32_t* p32 = _round_keys; p32 < _round_keys + GOST_BLOCK_LENGTH; ++p32) {
		for (uint8_t i = 0; i < 4; ++i) {
...
inline auto GostCore::_split_32bits_to_8bits(uint32_t block32b, uint8_t* blocks8b) -> void {
	for (uint8_t i = 0; i < 4; ++i) {
		blocks8b[i] = (uint8_t)(block32b >> (24 - (i * GOST_BLOCK_LENGTH)));
	}
}
...
GostCore::GostCore(const GostCore& gost) {
	std::memcpy(_round_keys, gost._round_keys, GOST_BLOCK_LENGTH);
	for (size_t c = 0; c < GOST_BLOCK_LENGTH; c++) {
		std::memcpy(_sbox[c], gost._sbox[c], 16);
	}
}
...
auto GostCore::set_substitution_tables(const uint8_t** sbox, const uint8_t** inv_sbox) -> void {
	...
	for (size_t c = 0; c < GOST_BLOCK_LENGTH; c++) {
		std::memcpy(_sbox[c], sbox[c], 16);
	}
}

auto GostCore::get_block_length() -> size_t {
	return GOST_BLOCK_LENGTH;
}
